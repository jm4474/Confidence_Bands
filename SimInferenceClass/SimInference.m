...
% ------------------------------------------------------------------------------
    ...
    %% 3)Critical Values for Rectangular Bands:
        ...
        function [suptplugin]=suptcritval_plugin(confidence,Sigma,I)
            ...
            d=diag(Sigma).^(.5);   % Extracts the diagonal of Vartheta
            d_nonzero = (d>eps); % Elements with nonzero variance
            Corrmat=bsxfun(@ldivide, d(d_nonzero), bsxfun(@rdivide, Sigma(d_nonzero,d_nonzero), d(d_nonzero)')); %Matrix of Correlations
        ...
        %% 7) Equal-tailed Confidence Band based on the quantiles of mudraws
